...
prev = dict() # predecessor of v

while Q:
    _, (v,d) = heapq.heappop(Q) # remove and return the best vertex

    if v == end: last = (v,d); break # stop when we reached end, end in my example is only reached from one previous position (does not work for all inputs)
    ...
    for u, weight in neighbor: # go through all u neighbors of v
        if u[0] in path:
            ...
            if alt < dist[u]: 
                prev[u] = {((v,d))} # relaxation
                dist[u] = alt
                heapq.heappush(Q, (alt, u))
            elif alt == dist[u]:
                prev[u].add((v,d)) # found same-distance path

todo = [last]
path = {(last)}
while todo[0][0] != start:
    for v in prev[todo.pop(0)]:
        todo.append(v)
        path.add(v[0])
